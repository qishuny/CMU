...
def init_particles_random(num_particles, occupancy_map):
    ...
    x0_vals = np.random.uniform(3500, 4500, (num_particles, 1))
    theta0_vals = np.random.uniform(-3.14, 3.14, (num_particles, 1))

    # x0_vals = np.full((num_particles, 1),4000)
    # y0_vals = np.full((num_particles, 1),5000)
    # theta0_vals = np.full((num_particles, 1),0)
...
def init_particles_freespace(num_particles, occupancy_map):
    ...
    yfree, xfree = np.where(occupancy_map == 0)

    sizex = len(xfree)
    
    index = np.random.choice(sizex-1, num_particles)

    x0_vals = xfree[index]* 10
    y0_vals = yfree[index]* 10
    x0_vals = x0_vals.reshape(num_particles,1) 
    y0_vals = y0_vals.reshape(num_particles,1) 
    theta0_vals = np.random.uniform(-3.14, 3.14, (num_particles, 1))
